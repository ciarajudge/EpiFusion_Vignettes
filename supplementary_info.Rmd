...
Provide a timescaled phylogenetic tree to the model using the `<tree>`
...
`node_1[0.3442]`). This requires some preprocessing, the ape in R
package does the trick, or you can use the `prep_epifusion_data()`
function in `EpiFusionUtilities` that can do this conversion for you
too. We recognise this is inconvenient and removing this requirement to
allow normal nexus trees is on the top of our TODO list.
...
random walk in `priors`). You can learn more about the different options
for fitting beta through time
[here](https://github.com/ciarajudge/EpiFusion/wiki/Options-for-Fitting-Beta).
...
There are **a lot** of parameters to choose from for your analysis, and
...
For `looseformbeta` analyses types, which we recommend using most of the
time, you'll also need to provide an `initialBeta` parameter and
`betaJitter`, which gives particles their initial beta value, and
specifies the 'freedom' of the random walk. Those will look like this:
...
The previous examples showed had the rates as constant over time
(`stepchange = false`). When you introduce a step-change by changing the
...
Finally, let's assume we don't know the time of the step-change, and we
...
## Appendix 3 - Main EpiFusionUtilities Functions

### `load_raw_epifusion("path_to_outputfolder")`

-   *Description:* Loads an R list with the full posterior trajectory
    samples, parameter samples, likelihoods and acceptance rates for all
    chains.
-   *Input:* A filepath to an EpiFusion outputs folder
-   *Output:* A large list with the full EpiFusion output in an R
    compatible format.
...
-   *Description:* Plots the likelihood trace for all chains in an
    EpiFusion analysis
-   *Input:* Raw epifusion output object generated by
    `load_raw_epifusion()`
...
-   *Description:* Extracts the final epifusion posterior samples,
    discarding a certain proportion at the beginning of each chain as
    burn-in
-   *Input:* Raw epifusion output object generated by
    `load_raw_epifusion()` and proportion of each chain to discard as
    burn-in
-   *Output:* A large list with the EpiFusion posterior samples
    aggregated across chains (with burn-in discarded). Also has
    trajectory means and HPDs, and parameter convergence statistics.
...
## Appendix 5 - Paired Sampling Infrastructure

Specifying the case and genomic sequence sampling rates for EpiFusion is an important aspect of the parameterisation, however it should be noted that the these should be somewhat linked i.e. their proportion of the total infections will be related to their proportion of each other. With this in mind it is possible to parameterise the `psi` (genomic sequencing proportion of all cases) parameter as being linked to the `phi` parameterisation, where the fraction of sampled sequences as a proportion of cases over time is used in conjunction with the `phi` parameterisation, in place of specifying these parameters separately.
...
This data chunk inputs the below data into EpiFusion - weekly case incidence (totalling 1185 cases), and a phylogenetic tree of sequenced samples (59 sequences in total).

``` {r plot_incidence_and_tree, echo = FALSE, dpi=450, fig.height = 7}
...
``` {r get_proportion, echo = F, dpi= 450, fig.height = 3}
...
``` {r plot_test_run, echo = F, dpi= 450, fig.height = 2.5}
...
``` {r more_advanced_test, dpi= 450, fig.height = 2.5, echo = F}
...
\newpage
## Appendix 6 - Composite Prior Distributions
